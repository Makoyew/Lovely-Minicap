    ...
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          successMessage.value = `Plant successfully removed - ${game.name}`;
    ...
    <div class="game-list">
      <div class="game-card" v-for="game in games.data" :key="game.id">
        <div class="game-image">
            <img :src="getImageUrl(game)" alt="Plant image" class="fixed-image">
        </div>
        <div class="game-details">
          <h2 class="text-center game-title">{{ game.name }}</h2>
          <div class="info-item">
        ...
        <div class="button-container">
            <Link :href="`/games/edit/${game.id}`" class="px-4 py-2 mt-4 mb-4 font-bold rounded-md text-green">
